...
class CostStrain3(Cost):
    ...
    def getIDX0(self,patient=[], model_instance=[],vectorData=[]):
        ...
        if self.costIDX0==[]:
            ...
            if model_instance==[]:
                ...
                iOnsetStart = model_instance.onsetQRS() - 25
    ...
    def getStrainForComparison(self,measStrain,modStrain,measTime,modTime,patient,model_instance):
        ...
        GlobLVMeas = measStrain[:,:3].mean(1).min()/100 + 1
        ...
        measStrain[:,3:] = measStrain[:,3:] / 100

        measStrain[:,3:] = measStrain[:,3:] / estimatedStretchRatio
