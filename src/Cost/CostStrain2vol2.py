...
class CostStrain2vol2(Cost):
    def __init__(self,costIDX0=[]):
        self.name = 'Strain2vol1a'
    ...
    def getCost2(self,patient=[],model_instance=[],vectorData=[]):
        ...
        meanRVmod = modStrain[:,:3].mean(1)
        idxRel50mod = np.argwhere((range(len(meanRVmod))>np.argmin(meanRVmod)) & (meanRVmod>.5*np.min(meanRVmod)))[0][0]


        # Get Strain for comparison, resample model to measured temporal resolution and remove diastole part
        measStrain1,modStrainDiscretized = self.getStrainForComparison(measStrain, modStrain,patient.RVtime,modelTime)
        ...
        e2 = np.mean(( (modStrainDiscretized-measStrain1)/strainConst)  **2) + ((modelTime[-1] - patient.RVtime[-1])/ timeConst)**2
        e2 = 1/3 * np.mean( ( (modStrainDiscretized[:,:2]-measStrain1[:,:2])/strainConst)  **2) +  2/3 * np.mean( ( (modStrainDiscretized[:,2:]-measStrain1[:,2:])/strainConst)  **2)   + np.max([0,((modelTime[-1] - patient.RVtime[-1])/ timeConst)**2-1])

        # EDV & ESV
        if not model_instance==[]:
            # LV EDV
            if 'MRI_LVEDV' in patient.scalarData:
                measEDV =  patient.scalarData['MRI_LVEDV']
            elif 'RVSV_MRI' in patient.scalarData:
                measEDV = patient.scalarData['RVSV_MRI'] / patient.scalarData['MRI_LVEF'] * 100
            elif 'LVEDV_Echo' in patient.scalarData:
                measEDV = patient.scalarData['LVEDV_Echo']
            else:
                measEDV = np.nan
                error
            modEDV = np.max(model_instance.getVector('Lv','Cavity','Lv','V'))
            e2 = e2 + ( ( measEDV - modEDV * 1e6  ) / 10 )**2

            # RV EDV
            if 'MRI_RVEDV' in patient.scalarData:
                measEDV =  patient.scalarData['MRI_RVEDV']
            elif 'RVSV_MRI' in patient.scalarData:
                measEDV = patient.scalarData['RVSV_MRI'] / patient.scalarData['MRI_RVEF'] * 100
            elif 'RVEDV_Echo' in patient.scalarData:
                measEDV = patient.scalarData['RVEDV_Echo']
            else:
                measEDV = np.nan
                print(patient.scalarData.keys())
                print( patient.scalarData['MRI_RVEDV'])
                error
            modEDV = np.max(model_instance.getVector('Rv','Cavity','Rv','V'))
            e2 = e2 + ( ( measEDV - modEDV * 1e6  ) / 10 )**2

        # Check for too much difference between idxRel50
        measRel50 = patient.RVtime[len(measStrain1)]
        modRel50 = modelTime[idxRel50mod]
        if np.abs(measRel50-modRel50)>0.1:
            e2=e2+((measRel50-modRel50)/0.1)**2

        # Check for unrealistic mlap
        useMLAPlim = True
        if useMLAPlim:
            mLAPlim = 25 #mmHg
            if not model_instance==[]:
    ...
    def getIDXRel50(self,measStrain):
        ...
        #if len(np.argwhere((range(len(meanRV))>idxMin) & (meanRV>.5*np.min(meanRV))))==0:
            #print(meanRV)
            #plt.plot(meanRV)
            #print(idxMin)
            #plt.show()
    ...
    def getStrainForComparison(self,measStrain,modStrain,measTime,modTime):
        ...
        # Normalize LV strain
        GlobLVMeas = measStrain[:,3:5].mean(1).min()
        GlobLVMod  = modStrain[:,3:5].mean(1).min()


        GlobRVMeas = measStrain[:,:3].mean(1).min()
        GlobRVMod  = modStrain[:,:3].mean(1).min()



        # Normalize model strain to measured strain
        #modStrainDiscretized = (modStrainDiscretized / 100 ) + 1
        modStrainDiscretized[:,3:5] = modStrainDiscretized[:,3:5] / GlobLVMod * GlobLVMeas
        modStrainDiscretized[:,:3] = modStrainDiscretized[:,:3] / GlobRVMod * GlobRVMeas
        #modStrainDiscretized = (modStrainDiscretized -1 ) * 100
