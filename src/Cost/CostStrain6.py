...
class CostStrain6(Cost):
    ...
    def measGetStrainOnset(self,patient):
        ...
        idxConAt = -int(np.ceil(0.075/patient.RVtime[1]))
    ...
    def getIDX0(self,patient=[], model_instance=[],vectorData=[]):
        ...
        if self.costIDX0==[]:
            ...
            if model_instance==[]:
                ...
                iOnsetStart = model_instance.onsetQRS() - 15
                iOnsetEnd = iOnsetStart + 31
            ...
            #plt.figure()
            #plt.plot((modStrain0/modStrain0[0,:]-1)*100)
            #plt.plot(measStrainOnset)
    ...
    def getCost2(self,patient=[],model_instance=[],vectorData=[]):
        ...
        '''
        #self.plotValidation(patient=patient,model_instance=model_instance,vectorData=vectorData)
        #plt.show()
        ...
        measStrain,modStrainDiscretized = self.getStrainForComparison(measStrain, modStrain,patient.RVtime,modelTime,patient,model_instance)
        # Calculate Error
        timeConst = 0.1
        strainConst = 1  / (patient.RVtime[1]*len(modStrainDiscretized))

        e2 = 1/3 * np.mean( ( (modStrainDiscretized[:,:2]-measStrain[:,:2])/strainConst)  **2) +  2/3 * np.mean( ( (modStrainDiscretized[:,2:]-measStrain[:,2:])/strainConst)  **2)   + ((modelTime[-1] - patient.RVtime[-1])/ timeConst)**2


        # Correct if idxRel50 does not match
        #strainDiff1 = ((modStrainDiscretized[ 0,:]-measStrain[ 0,:])/np.min(measStrain)*20)**2 - 1
        #strainDiff2 = ((modStrainDiscretized[-1,:]-measStrain[-1,:])/np.min(measStrain)*20)**2 - 1
        #strainDiff1[strainDiff1<0]=0
        #strainDiff2[strainDiff2<0]=0
        #e2 = e2 + np.sum(strainDiff1)+np.sum(strainDiff2)
    ...
    def getStrainForComparison(self,measStrain,modStrain,measTime,modTime,patient,model_instance):
        # Discretize Modelled Strain
        modStrainDiscretized = np.ndarray((len(measStrain),5))

        idxLastTime = int(np.ceil(0.075/measTime[1]))
        measStrain1 = np.concatenate(( measStrain[-idxLastTime:,:], measStrain[1:(-idxLastTime+1),:] ))

        for iSeg in range(5):
            modStrainDiscretized[:,iSeg] = np.interp(measTime-measTime[idxLastTime], np.concatenate((np.array(modTime)-modTime[-1],modTime[1:])), np.concatenate((modStrain[:,iSeg],modStrain[1:,iSeg])))


        idxRel50 = self.getIDXRel50(measStrain1)

        # do not normalize
        if False:
            # Normalize RV strain: change model amplitude to measured amplitude
            GlobLVMeas = measStrain1[:,:3].mean(1).min()/100 + 1
            GlobLVMod  = modStrain[:,:3].mean(1).min()/100 + 1

            modStrainDiscretized[:,:] = (modStrainDiscretized[:,:] / 100 )
            modStrainDiscretized[:,:3] = modStrainDiscretized[:,:3] / (GlobLVMod-1) * (GlobLVMeas-1)
            modStrainDiscretized[:,:] = (modStrainDiscretized[:,:] ) * 100

            # Normalize LV strain: use volumina
            #  - assume LV is a sphere
            LVVWall = model_instance.getScalar('Lv','Wall','Lv','VWall')+model_instance.getScalar('Sv','Wall','Sv','VWall')
            #print('LVVWall: ', LVVWall)
            LV_EDV = patient.scalarData['MRI_LVEDV'] * 1e-6
            LV_ESV = LV_EDV - model_instance.getScalar('','','','q0') * model_instance.getScalar('','','','tCycle')
            #print('LV_EDV: ', LV_EDV)
            #print('LV_ESV: ', LV_ESV)

            estimatedStretchRatio = ( (1 + 3 * LV_EDV / LVVWall) / (1 + 3 * LV_ESV / LVVWall) ) ** (1/3)
            #print('estimatedStretchRatio',estimatedStretchRatio)

            measStrain1[:,3:] = measStrain1[:,3:] / 100
            measStrain1[:,3:] = measStrain1[:,3:] / estimatedStretchRatio
            measStrain1[:,3:] = (measStrain1[:,3:]) * 100

        return measStrain1[:idxRel50,:],modStrainDiscretized[:idxRel50,:]
    ...
    def plotValidation(self,patient=[],model_instance=[],vectorData=[]):
        ...
        measStrain,modStrain = self.getStrainForComparison(measStrain,modStrain,measTime,modTime,patient,model_instance)
